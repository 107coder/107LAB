...
<html>
	...
	<body>
		...
		<div style="width:780px;height:150px;position:relative;top:130px;left:20px; ">
			...
			<div style="position:relative;left:300px;top:-10px;">发布时间：<?php $sql = "select addtime from news where id=57;"; $result = $mysqli->query($sql);while($row = mysqli_fetch_assoc($result)){echo "<td>{$row['addtime']}</td>";}; ?></div>
		</div>	
		
		<p><?php $sql = "select content from news where id=57;"; $result = $mysqli->query($sql);while($row = mysqli_fetch_assoc($result)){echo "<td>{$row['content']}</td>";}; ?></p>

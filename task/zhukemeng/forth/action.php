...
//（2）、连接MySQL、并选择数据库
	...
	mysqli_set_charset($link2,'utf8');    //!!!!!!在这里设置下字字符集编码就可以了，需要和数据库的字符集相同才可以插入成功，
	...
	switch($_GET["action"]){
		case "add": //执行添加操作
			//1. 获取要添加的信息，并补充其他信息
 			...
 			//2. 做信息过滤（省略）
			//3. 拼装添加SQL语句，并执行添加操作
				$sql = "insert into news (title,author,addtime,content) values('{$title}','{$author}','{$addtime}','{$content}');";
				//echo $sql;die;
